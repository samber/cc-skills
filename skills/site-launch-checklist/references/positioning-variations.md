# Positioning Variations by Directory Tier

Never copy-paste the same description everywhere. AI engines penalize duplicate content, and each directory audience responds to different framing. Prepare distinct variants per tier before submitting.
...
5. Analytics/reporting view
6. Team/collaboration features (if applicable)
...
- 45-60s: Key differentiator close-up
- 60-75s: Social proof / testimonials
- 75-90s: CTA + URL
...
We built this because [personal story — 2 sentences].

The short version: [1-sentence value prop].

What's different:
- [Differentiator 1 with technical detail]
- [Differentiator 2 with user benefit]
- [Differentiator 3 with outcome]

We're early — [honest state: "in beta," "pre-revenue," "100 users"]. Would love your honest take on the positioning, the UX, or the roadmap. What would you build next?

Try it: [direct signup link — no landing page redirect]
```
...
- [Advantage 1]: [specific detail + quantified benefit]
- [Advantage 2]: [specific detail + quantified benefit]
- [Advantage 3]: [specific detail + quantified benefit]

Best for: [ICP 1], [ICP 2], [ICP 3]
Pricing: [pricing model — e.g., "Free for up to 5 users, then $X/user/mo"]
Integrations: [top 5 integrations]

See honest comparison: [yourdomain.com/alternatives/competitor]
```
...
AI Capabilities:
- [Capability 1]: [specific AI feature — e.g., "autonomous agent that researches competitors"]
- [Capability 2]: [specific AI feature — e.g., "RAG over your codebase for context-aware suggestions"]
- [Capability 3]: [specific AI feature — e.g., "fine-tuned model for [domain] tasks"]

Human-in-the-loop: [yes/no — how humans review/approve AI output]
Data privacy: [local processing / encrypted / no training on your data]
Model flexibility: [BYOK / multiple providers / locked to one]
...
Protocol: [MCP / OpenAI Functions / LangChain Tools / CrewAI Skills / AutoGPT Commands]
Transport: [stdio / SSE / HTTP / WebSocket]
Authentication: [API key / OAuth / none / mTLS]

Capabilities exposed:
- [Capability 1]: [function name] — [description + params]
- [Capability 2]: [function name] — [description + params]
- [Capability 3]: [function name] — [description + params]

Installation: [one-liner install command]
...
What you can build:
- [Use case 1] — [template available: yes/no]
- [Use case 2] — [template available: yes/no]
- [Use case 3] — [template available: yes/no]

Features:
- [Feature 1: e.g., "Visual workflow builder with 50+ pre-built blocks"]
- [Feature 2: e.g., "One-click deploy to [platform]"]
- [Feature 3: e.g., "Built-in auth, database, and API"]

Templates included: [number] templates for [use cases]
Learning curve: [minutes/hours to first working app]
Community: [Discord/Slack/Forum size]
```
...
Technical approach:
- [Architecture decision 1]: [why + tradeoff]
- [Architecture decision 2]: [why + tradeoff]
- [Architecture decision 3]: [why + tradeoff]

Key features:
- [Feature 1]: [technical detail — e.g., "Zero-allocation hot path via arena allocator"]
- [Feature 2]: [technical detail — e.g., "Compile-time SQL validation via sqlc"]
- [Feature 3]: [technical detail — e.g., "Automatic retry with exponential backoff + jitter"]

Performance: [benchmarks — e.g., "3× faster than X, 50% less memory"]
Dependencies: [minimal / zero / listed]
License: [MIT / Apache 2.0 / BSD-3]
MSRV / Minimum version: [e.g., Go 1.21+, Python 3.10+]
...
Business Outcomes:
- [Outcome 1]: [metric — e.g., "Reduce onboarding time from 2 weeks to 2 hours"]
- [Outcome 2]: [metric — e.g., "Increase trial-to-paid conversion by 40%"]
- [Outcome 3]: [metric — e.g., "Save 15 hrs/week per CSM on manual tasks"]
...
Integrations: [CRM, marketing automation, analytics, support — top 10]
Security: [SOC 2 / ISO 27001 / GDPR / HIPAA — certifications]
Support: [SLA, channels, response time]
Implementation: [time to value — e.g., "Live in 30 minutes"]

Customers: [Company types — e.g., "200+ companies including Series A–C SaaS"]
Pricing: [Transparent pricing URL or "Contact for enterprise"]
```
...
| 4 Agent/MCP | Agent/MCP Angle | Agent developers | Protocol-focused, integration-ready | MCP server, function schemas |
| 5 No-Code | Ease + Power | Citizen developers | Accessible, visual, outcome-focused | Time to first app, templates |
| 6 Dev | Technical Depth | Library evaluators | Precise, spec-driven | Performance, architecture, types |
| 7 B2B Review | ROI + Use Case | Buyers, procurement | Professional, proof-heavy | Quantified business outcomes |
...
7. **Generic category tags** → Use specific tags the directory's search uses
8. **Broken demo video link** → Test every URL before submitting
9. **Mismatched pricing** → Directory pricing must match your live pricing page
10. **Outdated screenshots** → Must match current UI exactly