...
7. **Write findings as prose paragraphs**, not bullet lists. Embed figures in sentences:
   ...
   - ✗ "* Market: $4.2B"
   Bullets only for true enumerated lists (product names, compliance items, steps).
...
11. **Write immediately to your output file** — do not batch at the end. The file path is:
    `{absolute path to research/{date}-{type}-{topic}/{axis}.md}`
