# Playbooks — runbooks for specific situations

This file is a tactical handbook for high-leverage moves across domains. Each section is a runbook: trigger → steps → scripts → failure signals.

The frames behind these plays draw on multiple modern selling traditions (value-message frameworks, the JOLT no-decision protocol, MEDDPICC, gap-selling current/future state, mutual-action-plan operationalisation, and the broader negotiation process spine). The runbooks are framework-agnostic — pick the one that fits the situation, not the one you read most recently.

## 1. Champion test — is your "champion" actually a coach?
...
**The 3-question commitment test** (see [references/discovery.md](discovery.md#champion-validation--the-3-question-commitment-test)):

1. _"How will we know we're on target?"_
2. _"What will we do if we're not?"_
3. _"How are we going to close this?"_

A real champion answers each with specifics and named people. A coach generalises, hedges, or pivots.
...
## 2. Multi-threading sequence — from 1 contact to 4–7 stakeholders
...
## 3. Mutual Action Plan (MAP) — the close timeline as artifact
...
**Failure signal.** The buyer refuses to put dates on the MAP, or repeatedly defers them by 1–2 weeks each time. Either MEDDPICC is incomplete (no real Decision Process visibility) or you're being JOLT-trapped (see below).

## 4. JOLT — the no-decision protocol
...
## 5. Executive sponsor (EB) engagement — the 5-minute opening
...
## 6. Renewal & expansion — the 90-day coopétition cadence
...
Multi-thread (see Play 2 above) BEFORE the renewal. New stakeholders since the original deal — new EB, new CFO, new procurement lead. They didn't sign the original; their priors aren't aligned with renewal.
...
## 7. Pre-call preparation runbook
...
## 8. Salary ask — the structured raise / offer conversation
...
## 9. Decision announcement / difficult 1:1
